export function searchAnagram(str, target) {
  ...
  const flatPrepare = Array.from(link.keys()).flat().sort((a, b) => a - b);
  ...
  for(let i = 0; i < flatPrepare.length; i++){
    ...
    while(flatPrepare[start] + 1 === flatPrepare[start + 1] && part.length < sizeTarget && !partHash.has(link.get(flatPrepare[start + 1]))){
